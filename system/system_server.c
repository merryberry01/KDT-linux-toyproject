...
void *watchdog_thread(void* arg)
{
    ...
    /* 여기에 구현하세요. */
    while (1) {
	    if((mqretcode = mq_receive(watchdog_queue, (char *)&msg, sizeof(msg), NULL)) == -1)
		    perror("mq_receive() from watchdog_thread()");
    }
...
void *monitor_thread(void* arg)
{
    ...
    /* 여기에 구현하세요. */
    while (1) {
	    if((mqretcode = mq_receive(monitor_queue, (char *)&msg, sizeof(msg), NULL)) == -1)
		    perror("mq_receive() from monitor_thread()");
    }
...
void *disk_service_thread(void* arg)
{
    ...
    /* 여기에 구현하세요. */
    while (1) {
	    if((mqretcode = mq_receive(disk_queue, (char *)&msg, sizeof(msg), NULL)) == -1)
		    perror("mq_receive() from disk_thread()");
    }
...
void *camera_service_thread(void* arg)
{
    ...
    while (1) {
	    if((mqretcode = mq_receive(camera_queue, (char *)&msg, sizeof(msg), NULL)) == -1)
		    perror("mq_receive() from camera_thread()");
...
int system_server()
{
    ...
    /* 여기에 구현하세요. */
    /* 메시지 큐를 오픈한다. */
    if((watchdog_queue = mq_open("/watchdog_queue", O_RDWR)) == -1)
	    perror("mq_open(watchdog_queue) from system_server()");
    if((monitor_queue = mq_open("/monitor_queue", O_RDWR)) == -1)
	    perror("mq_open(monitor_queue) from system_server()");
    if((disk_queue = mq_open("/disk_queue", O_RDWR)) == -1)
	    perror("mq_open(disk_queue) from system_server()");
    if((camera_queue = mq_open("/camera_queue", O_RDWR)) == -1)
	    perror("mq_open(camera_queue) from system_server()");
